...
def test_settings_loading():
    ...
    try:
        ...
        print(f"  LLM Anthropic key (from config): {'✓ Set' if settings.llm.get('anthropic_api_key') else '✗ Not set'}")
        print(f"  LLM OpenAI key (from config): {'✓ Set' if settings.llm.get('openai_api_key') else '✗ Not set'}")
        print(f"  Telemetry enabled: {settings.telemetry.get('enabled', False)}")
        
        ga_config = settings.telemetry.get('google_analytics', {})
        if ga_config:
            print(f"  GA Measurement ID (from config): {ga_config.get('measurement_id', 'Not set')}")
            print(f"  GA API Secret (from config): {'✓ Set' if ga_config.get('api_secret') else '✗ Not set'}")
        else:
