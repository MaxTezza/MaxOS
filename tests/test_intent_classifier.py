...
def mock_planner():
    ...
    planner.plan.return_value = Intent(name="system.general", confidence=0.2, slots=[], summary="General system request")
    return planner
...
async def test_classify_fallback_to_planner(intent_classifier, mock_planner):
    ...
    mock_planner.plan.return_value = Intent(name="file.list", confidence=0.65, slots=[], summary="List directory contents")
