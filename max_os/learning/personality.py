...
class UserPersonalityModel:
    ...
    def _update_communication_style(self, interaction: Interaction) -> None:
        ...
        if interaction.user_reaction == "positive":
            ...
            self.verbosity_preference += self.learning_rate * (
                response_verbosity - self.verbosity_preference
            )

            # Reinforce technical level
            self.technical_level += self.learning_rate * (
                interaction.technical_complexity - self.technical_level
            )
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                for hour, task, freq in temporal_data:
                    ...
                    self.temporal_patterns[hour].append({"task": task, "frequency": freq})
    ...
    def predict_next_need(self, context: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        if features["git_has_staged"]:
            add_prediction(
                "push commits", 0.82, "There are staged changes ready to push", "context"
            )

        if features["active_app_is_ide"]:
            add_prediction(
                "show git status", 0.74, "Active window looks like a coding session", "context"
            )

        if features["recent_python_file_modified"]:
            add_prediction("run tests", 0.78, "Recent Python files were modified", "context")

        if features["new_downloads_recently"]:
            add_prediction(
                "organize downloads", 0.72, "New files landed in Downloads recently", "context"
            )
    ...
    def _extract_features(self, context: dict[str, Any]) -> dict[str, Any]:
        ...
        features["git_has_staged"] = (
            1 if any(repo.get("staged") for repo in git_info.get("repos", [])) else 0
        )
        ...
        features["active_app_is_ide"] = (
            1
            if any(
                keyword in active_window
                for keyword in ["code", "pycharm", "vim", "idea", "studio", "sublime"]
            )
            else 0
        )
        features["active_app_is_browser"] = (
            1
            if any(keyword in active_window for keyword in ["chrome", "firefox", "edge", "brave"])
            else 0
        )

        # Filesystem activity
        filesystem_info = signals.get("filesystem", {})
        features["recent_python_file_modified"] = (
            1
            if any(
                event.get("src_path", "").endswith(".py")
                for event in filesystem_info.get("recent_events", [])
            )
            else 0
        )
        features["new_downloads_recently"] = (
            1
            if (
                filesystem_info.get("downloads")
                and self._is_recent_timestamp(filesystem_info["downloads"][0].get("modified"))
            )
            else 0
        )
        ...
        # Last action
        features["last_action_is_git_status"] = (
            1 if context.get("last_action", "").lower() == "show git status" else 0
        )
        features["last_action_is_commit"] = (
            1 if "commit" in context.get("last_action", "").lower() else 0
        )
